...
import type { ThemeAccentColor, ThemeMode } from "./theme.types.js";
 ...
 *     description: >
 *       尚未設定過的欄位回傳系統預設值（mode: SYSTEM, accentColor: BLUE），不是寫死在使用者資料裡的快照——
 ...
 *     description: mode／accentColor 皆選填，但至少要給一個；沒給的欄位維持原本設定（或系統預設）不變。
 *     tags:
 *       - User
 *     security:
 *       - bearerAuth: []
 ...
 *       400:
 *         description: mode/accentColor 都沒給，或值不在允許的選項內。
 *       401:
 *         description: 缺少或無效的 Authorization header / token。
 */
userRouter.put("/me/theme", requireAuth, async (req: AuthenticatedRequest, res) => {
  const firebaseUid = requireUser(req);
  const body = req.body as { mode?: unknown; accentColor?: unknown } | null;
